...
#include "me.h"
#include "time.h"
#if SSIM_QUANTIZATION_SIMD || QPA_SIMD
#include <mmintrin.h>
#include <emmintrin.h>
#include <tmmintrin.h>
#include <smmintrin.h>
...
static void init_gauss_coeff(int* coef)
{
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            ...
            d_gauss[i * N + j] = exp((0 - distance) / (2 * sigma * sigma)) / 2 * PI * sigma * sigma;
        }
    }
...
{
    //if (h->param.rc.i_aq_mode != 0)//看ultrafast模式下是否调用adaptiveQP
    //{
    //    printf("\n1\n");
    //}
    if (h->param.QPA1 == 1) 
...
#define com_malloc_fast(size)     com_malloc((size))
        int ctu_in_width = (frame->i_width[0] / QPA_SIZE) + ((frame->i_width[0] % QPA_SIZE) == 0 ? 0 : 1);//lcu横向
        int ctu_in_height = frame->i_lines[0] / QPA_SIZE + ((frame->i_lines[0] % QPA_SIZE) == 0 ? 0 : 1);//lcu纵向
        int num_lcu = ctu_in_width * ctu_in_height;
        frame->m_iOffsetCtu = com_malloc_fast(num_lcu * sizeof(int8_t));
        memset(frame->m_iOffsetCtu, 0, num_lcu * sizeof(int8_t));
        memset(frame->f_qpa_qp_offset, 0, h->mb.i_mb_count * sizeof(float));
        //int num = frame->i_width[0] * frame->i_lines[0];
        //frame->temp = com_malloc_fast(num * sizeof(pel));
        //memset(frame->temp, 0, num * sizeof(pel));
        


        applyQPAdaptation(frame);


        for (int mb_y = 0; mb_y < h->mb.i_mb_height; mb_y++)
            for (int mb_x = 0; mb_x < h->mb.i_mb_width; mb_x++)
            {
                int QPA_x = mb_x * 16 / QPA_SIZE;//QPA块坐标，以宏块左上角像素点坐标为例
                int QPA_y = mb_y * 16 / QPA_SIZE;
                int mb_index = mb_x + mb_y * h->mb.i_mb_stride;
                int QPA_index = QPA_x + QPA_y * ((h->param.i_width / QPA_SIZE) + (h->param.i_width % QPA_SIZE) == 0 ? 0 : 1);//宽度不一定能整除
                //frame->f_qp_offset[mb_index] = frame->m_iOffsetCtu[QPA_index];
                frame->f_qpa_qp_offset[mb_index] = frame->m_iOffsetCtu[QPA_index];
        ...
        /*if (SSIM1 == 1)
        {
            ...
            ssimqt_init(h->param.i_width, h->param.i_height);
            ssimqt_factor(frame->plane[0], h->param.i_width, h->param.i_height, frame->i_stride[0]);

            memset(frame->f_ssim_qp_offset, 0, h->mb.i_mb_count * sizeof(float));
            memset(frame->f_lambda_factor, 0, h->mb.i_mb_count * sizeof(float));
            int aq_width = h->param.i_width / SSIM_AQ_SIZE;
            int aq_height = h->param.i_height / SSIM_AQ_SIZE;
            for (int ssim_mb_y = 0; ssim_mb_y < aq_height; ssim_mb_y++)
                for (int ssim_mb_x = 0; ssim_mb_x < aq_width; ssim_mb_x++)
                {
                    int ssim_mb_index = ssim_mb_x + ssim_mb_y * aq_width;
                    frame->f_ssim_qp_offset[ssim_mb_index] = ssimqt_qpoffset(ssim_mb_x * SSIM_AQ_SIZE, ssim_mb_y * SSIM_AQ_SIZE, SSIM_AQ_SIZE, SSIM_AQ_SIZE, 35, &frame->f_lambda_factor[ssim_mb_index]);
                }
        }*/
    ...
    /* Degenerate cases */
    if (h->param.rc.i_aq_mode == X264_AQ_NONE || h->param.rc.f_aq_strength == 0)//0
    {
    ...
    else//开启AQ
    {
        ...
        if (h->param.rc.i_aq_mode == X264_AQ_AUTOVARIANCE || h->param.rc.i_aq_mode == X264_AQ_AUTOVARIANCE_BIASED)//1  2
        {
        ...
        for (int mb_y = 0; mb_y < h->mb.i_mb_height; mb_y++)
            for (int mb_x = 0; mb_x < h->mb.i_mb_width; mb_x++)
            {
                ...
                if (h->param.rc.i_aq_mode == X264_AQ_AUTOVARIANCE_BIASED)//3
                {
                    qp_adj = frame->f_qp_offset[mb_xy];
                    qp_adj = strength * (qp_adj - avg_adj) + bias_strength * (1.f - 14.f / (qp_adj * qp_adj));
                }
                else if (h->param.rc.i_aq_mode == X264_AQ_AUTOVARIANCE)//2
                {
                    qp_adj = frame->f_qp_offset[mb_xy];
                ...
                //上面计算出x264原本的qp_adj,下面是test 


                if (h->param.QPA1 == 1 || h->param.SSIM1 == 1) {

                    float qp_adj_ssim = 0;
                    float qp_adj_qpa = 0;
                
                    if (h->param.QPA1 == 1) {
                        qp_adj_qpa = frame->f_qpa_qp_offset[mb_xy];
                    }

                    if (h->param.SSIM1 == 1) {

                        int ssim_mb_x = (mb_x * 16) / SSIM_AQ_SIZE;
                        int ssim_mb_y = (mb_y * 16) / SSIM_AQ_SIZE;
                        int ssim_mb_index = ssim_mb_x + ssim_mb_y * aq_width_temp;
                        qp_adj_ssim = frame->f_ssim_qp_offset[ssim_mb_index];
                    }
                    if (h->param.QPA1 == 1 && h->param.SSIM1 == 1)
                    {//qpa 1   ssim 1   取较小值
                        if (qp_adj_ssim >= qp_adj_qpa)
                        {
                            qp_adj += x264_clip3(qp_adj_qpa, -2, 4);
                        }
                        else
                            qp_adj += x264_clip3(qp_adj_ssim, -2, 4);

                    }
                    if (h->param.QPA1 == 1 && h->param.SSIM1 == 0) {//qpa1  ssim  0
                        qp_adj += x264_clip3(qp_adj_qpa, -2, 4);
                    }

                    if (h->param.QPA1 == 0 && h->param.SSIM1 == 1) {//qpa 0  ssim  1
                        qp_adj += x264_clip3(qp_adj_ssim, -2, 4);
                    }

                    //qp_adj += x264_clip3(qp_adj_ssim, -3, 3);
    ...
    //#if SSIM_QUANTIZATION || ENABLE_QPA
    //    h->param.rc.i_aq_mode = X264_AQ_VARIANCE;
    //#endif

    if (h->param.QPA1 == 1) {    //释放内存空间
#define com_mfree(m)              if(m){free(m);}
    //h->param.rc.i_aq_mode= aq_mode_temp;
    com_mfree(frame->m_iOffsetCtu);
    //com_mfree(frame->temp);
}
...
ssimquant_t g_ssimqt;

inline static int ssimqt_variance(uint8_t* p, int w, int h, int stride)
{
    int             iMean = 0;
    int             iVar = 0;
    const uint8_t* pTmp = p;
    for (int j = 0; j < h; j++, p += stride - w)
    {
        for (int i = 0; i < w; i++)
        {
            iMean += *p++;
    ...
    for (int j = 0; j < h; j++, pTmp += stride - w)
    {
        for (int i = 0; i < w; i++)
        {
            int iDif = *(pTmp++) - iMean;
...
void ssimqt_init(int width, int height)
{
    int w = width / MIN_SSIM_AQ_SIZE;
    int h = height / MIN_SSIM_AQ_SIZE;
    if (g_ssimqt.afFactor == NULL)
    {
        g_ssimqt.afFactor = (double*)malloc(sizeof(double) * (w * h));
        memset(g_ssimqt.afFactor, 0, sizeof(double) * w * h);
        g_ssimqt.iStride = g_ssimqt.iWidth = w;
        g_ssimqt.iHeight = h;
    }
    else
    {
...
#if SSIM_QUANTIZATION_SIMD
static void ssimqt_gaussian_filter_avx2(uint8_t* dst, int i_dst, uint8_t* src, int i_src, int* coef, int lcu_width, int lcu_height)
{
...
__inline static void ssimqt_gaussian_filter(uint8_t* p_src, int width, int height, int stride, uint8_t* p)
{
    const int       N = 7;
    const double    sigma = 1.54089642;

    uint8_t* p_fenc = p_src;
    int             i_stride = stride;
    int             i_w = width;
    int             i_h = height;
    int             radius = N / 2;
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            double distance = (radius - i) * (radius - i) + (radius - j) * (radius - j);
            double temp = exp((0 - distance) / (2 * sigma * sigma)) / 2 * PI * sigma * sigma;
            d_sum += temp;
            d_gauss[i * N + j] = temp;
        }
    }
    ...
    for (int j = radius; j < i_h - radius; j++)
    {
        for (int i = radius; i < i_w - radius; i++)
        {
            ...
            for (int m = j - radius; m <= j + radius; m++)
            {
                ...
                {
                    sum += p_fenc[m * i_stride + n] * d_gauss[index++];
                }
            }
            if (sum > 255)
                sum = 255;
            p[j * i_stride + i] = (uint8_t)sum;
        }
    }
    free((void*)d_gauss);
}
...
void ssimqt_factor(uint8_t* p_src, int width, int height, int stride)
{
    int           iStride = stride;
    int           iW = width / MIN_SSIM_AQ_SIZE;
    int           iH = height / MIN_SSIM_AQ_SIZE;

    double        iMean = 0.0;
    const float   fC2 = 58.5225f;

    uint8_t* pY = (uint8_t*)malloc(sizeof(uint8_t) * stride * height);
    memcpy(pY, p_src, sizeof(uint8_t) * stride * height);

#if SSIM_QUANTIZATION_SIMD
    ssimqt_gaussian_filter_avx2(pY, stride, p_src, stride, g_gauss_coeff, width, height);
#else
    ssimqt_gaussian_filter(p_src, width, height, stride, pY);
#endif
    ...
    for (int j = 0; j < iH; j++)
    {
        for (int i = 0; i < iW; i++)
        {
            int iOffset = i * MIN_SSIM_AQ_SIZE + j * MIN_SSIM_AQ_SIZE * iStride;
            for (int jj = 0; jj < 2; jj++)
            {
                for (int ii = 0; ii < 2; ii++)
                {
                    int i_offset_mb = iOffset + ii * 4 + jj * 4 * iStride;

                    int var = ssimqt_variance(pY + i_offset_mb, 4, 4, iStride);
                    g_ssimqt.afFactor[i + j * iW] += sqrt(var * 2 / 15 + fC2);
                }
            }
            g_ssimqt.afFactor[i + j * iW] /= 4;
        }
    }
    ...
    for (int j = 0; j < iH; j++)
    {
        for (int i = 0; i < iW; i++)
        {
            iMean += g_ssimqt.afFactor[i + j * iW];
        }
    }
    iMean /= iW * iH;
    ...
    for (int j = 0; j < iH; j++)
    {
        for (int i = 0; i < iW; i++)
        {
            g_ssimqt.afFactor[i + j * iW] /= iMean;
        }
    }
...
int ssimqt_qpoffset(int posx, int posy, int width, int height, int qp, float* factor)
{
    int     iX = posx / MIN_SSIM_AQ_SIZE;
    int     iY = posy / MIN_SSIM_AQ_SIZE;
    int     iW = width / MIN_SSIM_AQ_SIZE;
    int     iH = height / MIN_SSIM_AQ_SIZE;
    int     iIndex = iX + g_ssimqt.iStride * iY;

    if (iX + iW > g_ssimqt.iWidth)
    {
        iW = g_ssimqt.iWidth - iX;
    }
    if (iY + iH > g_ssimqt.iHeight)
    {
        iH = g_ssimqt.iHeight - iY;
    }

    double   fAvgFactor = 0.0;

    for (int j = 0; j < iH; j++)
    {
        for (int i = 0; i < iW; i++)
        {
            int iIndexIn = iIndex + i + g_ssimqt.iStride * j;
            fAvgFactor += g_ssimqt.afFactor[iIndexIn];
        }
    }
    fAvgFactor /= iW * iH;

    *factor = fAvgFactor;
    float qstep = (float)(pow(2.0, (double)(qp - 4) / 6) * fAvgFactor);
    int qpnew = (int)(log(qstep) / log(2.0) * 6 + 4);
    int qpoff = qpnew - qp;
    qpoff = x264_clip3(qpoff, -8, 8);

    g_ssimqt.iTmpOffset = qpoff;
...
int apprI4Log2(const double d) // rounded 3*log2(d)  3log2(wk)
{
#if global
    double temp = pow(d, 0.6);
    return d < 1.5e-13 ? -128 : (int)(floor(3.0 * log(temp) / log(2.0) + 0.5));
#else
...
////计算一个块图像主观变化剧烈成都ak
//void filterAndCalculateAverageEnergies(pel* pSrc, int srcStride, double* hpEner, int height, int width, int bitDepth)
//{
//    s64 saAct = 0;
//
...
//    pSrc += srcStride;
//#if QPA_SIMD
//    __m256i p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18;
//    __m256i m11, m12, m13, m14, m21, sa;
//    __m256i r1, r2, r3, r4, r5, r6, c1, c2, c3, c4, s11, s12;
//    const __m256i coeff_1 = _mm256_set1_epi16(12);    // E
//    const __m256i coeff_2 = _mm256_set1_epi16(-2);
//    int x, y;
//    s32 r_tmp[8];
//    __m256i ones = _mm256_set1_epi16(1);
//    __m256i s_final = _mm256_set1_epi32(0);
//    for (y = 1; (y + 3) < height - 1; y += 4)
//    {
//        __m256i sf = _mm256_set1_epi16(0);
//        for (x = 1; (x + 15) < width - 1; x += 16)   // and columns
//        {
//            p1 = (_mm256_loadu_si256((__m256i*)(pSrc + x - 1 - srcStride)));
//            p2 = (_mm256_loadu_si256((__m256i*)(pSrc + x - srcStride)));
//            p3 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 1 - srcStride)));
//            p4 = (_mm256_loadu_si256((__m256i*)(pSrc + x - 1)));
//            p5 = (_mm256_loadu_si256((__m256i*)(pSrc + x)));
//            p6 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 1)));
//            p7 = (_mm256_loadu_si256((__m256i*)(pSrc + x - 1 + srcStride)));
//            p8 = (_mm256_loadu_si256((__m256i*)(pSrc + x + srcStride)));
//            p9 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 1 + srcStride)));
//            p10 = (_mm256_loadu_si256((__m256i*)(pSrc + x - 1 + 2 * srcStride)));
//            p11 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 2 * srcStride)));
//            p12 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 1 + 2 * srcStride)));
//            p13 = (_mm256_loadu_si256((__m256i*)(pSrc + x - 1 + 3 * srcStride)));
//            p14 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 3 * srcStride)));
//            p15 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 1 + 3 * srcStride)));
//            p16 = (_mm256_loadu_si256((__m256i*)(pSrc + x - 1 + 4 * srcStride)));
//            p17 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 4 * srcStride)));
//            p18 = (_mm256_loadu_si256((__m256i*)(pSrc + x + 1 + 4 * srcStride)));
//
//            r1 = _mm256_add_epi16(p1, p3);
//            r2 = _mm256_add_epi16(p4, p6);
//            r3 = _mm256_add_epi16(p7, p9);
//            r4 = _mm256_add_epi16(p10, p12);
//            r5 = _mm256_add_epi16(p13, p15);
//            r6 = _mm256_add_epi16(p16, p18);
//
//            c1 = _mm256_add_epi16(p2, p8);
//            c2 = _mm256_add_epi16(p5, p11);
//            c3 = _mm256_add_epi16(p8, p14);
//            c4 = _mm256_add_epi16(p11, p17);
//
//            m11 = _mm256_mullo_epi16(coeff_1, p5);
//            m12 = _mm256_mullo_epi16(coeff_1, p8);
//            m13 = _mm256_mullo_epi16(coeff_1, p11);
//            m14 = _mm256_mullo_epi16(coeff_1, p14);
//
//            /*     Start the first row      */
//            s11 = _mm256_add_epi16(r2, c1);
//            m21 = _mm256_mullo_epi16(coeff_2, s11);
//            s11 = _mm256_add_epi16(r1, r3);
//            s12 = _mm256_add_epi16(m11, m21);
//            sa = _mm256_abs_epi16(_mm256_sub_epi16(s12, s11));
//            sf = _mm256_add_epi16(sf, sa);
//            /*     End the first row      */
//
//            /*     Start the second row      */
//            s11 = _mm256_add_epi16(r3, c2);
//            m21 = _mm256_mullo_epi16(coeff_2, s11);
//            s11 = _mm256_add_epi16(r2, r4);
//            s12 = _mm256_add_epi16(m12, m21);
//            sa = _mm256_abs_epi16(_mm256_sub_epi16(s12, s11));
//            sf = _mm256_add_epi16(sf, sa);
//            /*     End the second row      */
//
//            /*     Start the third row      */
//            s11 = _mm256_add_epi16(r4, c3);
//            m21 = _mm256_mullo_epi16(coeff_2, s11);
//            s11 = _mm256_add_epi16(r3, r5);
//            s12 = _mm256_add_epi16(m13, m21);
//            sa = _mm256_abs_epi16(_mm256_sub_epi16(s12, s11));
//            sf = _mm256_add_epi16(sf, sa);
//            /*     End the third row      */
//
//            /*     Start the fourth row      */
//            s11 = _mm256_add_epi16(r5, c4);
//            m21 = _mm256_mullo_epi16(coeff_2, s11);
//            s11 = _mm256_add_epi16(r4, r6);
//            s12 = _mm256_add_epi16(m14, m21);
//            sa = _mm256_abs_epi16(_mm256_sub_epi16(s12, s11));
//            sf = _mm256_add_epi16(sf, sa);
//            ///*     End the fourth row      */
//        }
//        sf = _mm256_madd_epi16(sf, ones);
//        s_final = _mm256_add_epi32(s_final, sf);
//
//        pSrc += 4 * srcStride;
//    }
//    _mm256_storeu_si256((__m256i*)r_tmp, s_final);
//    saAct += (r_tmp[0] + r_tmp[1] + r_tmp[2] + r_tmp[3] + r_tmp[4] + r_tmp[5] + r_tmp[6] + r_tmp[7]);
//#else
...
//extern __m256i __cdecl _mm256_set1_epi16(int16_t);
//extern __m256i __cdecl _mm256_set1_epi32(int32_t);
//extern __m256i __cdecl _mm256_cvtepi8_epi16(__m256i);
//extern __m256i __cdecl _mm_loadu_si128(__m128i*);
//extern __m256i __cdecl _mm256_loadu_si256(__m256i const*);
//extern __m256i __cdecl _mm256_add_epi16(__m256i, __m256i);
//extern __m256i __cdecl _mm256_mullo_epi16(__m256i, __m256i);
//extern __m256i __cdecl _mm256_abs_epi16(__m256i);
...
//        for (x = 1; (x + 15) < width - 1; x += 16)   // and columns
//        {
//            p1 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 - srcStride)));
//            p2 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - srcStride)));
//            p3 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 - srcStride)));
//            p4 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1)));
//            p5 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x)));
//            p6 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1)));
//            p7 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + srcStride)));
//            p8 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + srcStride)));
//            p9 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + srcStride)));
//            p10 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + 2 * srcStride)));
//            p11 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 2 * srcStride)));
//            p12 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + 2 * srcStride)));
//            p13 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + 3 * srcStride)));
//            p14 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 3 * srcStride)));
//            p15 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + 3 * srcStride)));
//            p16 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + 4 * srcStride)));
//            p17 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 4 * srcStride)));
//            p18 = _mm256_cvtepi8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + 4 * srcStride)));
//
//            r1 = _mm256_add_epi16(p1, p3);
//            r2 = _mm256_add_epi16(p4, p6);
//            r3 = _mm256_add_epi16(p7, p9);
...
void filterAndCalculateAverageEnergies(pel* pSrc, int srcStride, double* hpEner, int height, int width, int bitDepth)
{
    ...
    for (y = 1; (y + 3) < height - 1; y += 4)
    {
        ...
        for (x = 1; (x + 15) < width - 1; x += 16)   // and columns
        {
            p1 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 - srcStride)));
            p2 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - srcStride)));
            p3 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 - srcStride)));
            p4 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1)));
            p5 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x)));
            p6 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1)));
            p7 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + srcStride)));
            p8 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + srcStride)));
            p9 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + srcStride)));
            p10 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + 2 * srcStride)));
            p11 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 2 * srcStride)));
            p12 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + 2 * srcStride)));
            p13 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + 3 * srcStride)));
            p14 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 3 * srcStride)));
            p15 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + 3 * srcStride)));
            p16 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x - 1 + 4 * srcStride)));
            p17 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 4 * srcStride)));
            p18 = _mm256_cvtepu8_epi16(_mm_loadu_si128((__m128i*)(pSrc + x + 1 + 4 * srcStride)));
...
//计算一帧图像主观变化剧烈程度a_pic
double getAveragePictureEnergy(int width, int height, int bitDepth)
{
    const double hpEnerPic =
        16.0 * sqrt((3840.0 * 2160.0) / (double)(width * height)) * (double)(1 << (2 * bitDepth - 10));
...
void applyQPAdaptation(x264_frame_t* frame)
{
    //const int bitDepth = ctx->info.bit_depth_internal; // overall image bit-depth
    const int bitDepth = 8; // overall image bit-depth

    //const int  iQPIndex = h->sh.i_qp; // initial QP index for current slice, used in following loops   h->sh->i_qp_delta
    //const int  iQPIndex = 57;
    //BOOL   sliceQPModified = FALSE;
    //COM_PIC   pic_org = ctx->pico->pic;
    //int MAX_QP = MAX_QUANT_BASE + ctx->info.qp_offset_bit_depth;
    //int MAX_QP = MAX_QUANT_BASE + 16;

    int ctu_in_width = (frame->i_width[0] / QPA_SIZE) + ((frame->i_width[0] % QPA_SIZE) == 0 ? 0 : 1);
    int ctu_in_height = frame->i_lines[0] / QPA_SIZE + ((frame->i_lines[0] % QPA_SIZE) == 0 ? 0 : 1);
    int num_lcu = ctu_in_width * ctu_in_height;

    // mean visual activity value and luma value in each CTU

#if global 
    double hpEnerPic = 0;


    for (int ctuIdx = 0; ctuIdx < num_lcu; ctuIdx++)//给一帧中所有CTU计算aq_offset
    {
        int posx = (ctuIdx % ctu_in_width) * QPA_SIZE;//QPA的横坐标
        int posy = (ctuIdx / ctu_in_width) * QPA_SIZE;



        pel* curr_org = frame->plane[0] + (posx - (posx > 0 ? 1 : 0)) + (posy - (posy > 0 ? 1 : 0)) * frame->i_width[0];

        int stride = frame->i_stride[0];
        int last_x_size = ((posx + QPA_SIZE) > frame->i_width[0]) ? (frame->i_width[0] - posx) : QPA_SIZE;
        int last_y_size = ((posy + QPA_SIZE) > frame->i_lines[0]) ? (frame->i_lines[0] - posy) : QPA_SIZE;
        int width = last_x_size + (posx > 0 ? 2 : 1);
        int height = last_y_size + (posy > 0 ? 2 : 1);
        double hpEner = 0.0;
        filterAndCalculateAverageEnergies(curr_org, stride, &hpEner, height, width, bitDepth);

        hpEnerPic += hpEner;
        //int offset = COM_CLIP3(-3, 3, apprI4Log2(hpEner * hpEnerPic));//计算aq_offset
        ////int iQPAdapt = COM_CLIP3(0, MAX_QP, iQPIndex + offset);
        ////printf("\nctuIdx[%d]=%d\n", ctuIdx,iQPAdapt);
        //frame->m_iOffsetCtu[ctuIdx] = (int8_t)offset;//经过DEBUG，第5个出了问题，这个问题已经解决了，变量写错了
        ////printf("\n---%d\n", h->fenc->m_iOffsetCtu[ctuIdx]);
       
    }
    hpEnerPic /= num_lcu;
    hpEnerPic = 1.0 / hpEnerPic;
#else
    const double hpEnerPic = 1.0 / getAveragePictureEnergy(frame->i_width[0], frame->i_lines[0], bitDepth);  // inverse, speed
#endif
...
int x264_ratecontrol_mb_qp( x264_t *h )
{
    ...
    if (h->param.rc.i_aq_mode)
    {
        ...
        if (h->param.QPA1 == 1 || h->param.SSIM1 == 1) {  //把值传给qp_qpa_fixed
            qp_qpa_fixed = x264_clip3(qp + 0.5f, h->param.rc.i_qp_min, h->param.rc.i_qp_max);//把值传给qp_qpa_fixed,在这里第一个点的QP已经变成36了
        }
    }
