...
static void mb_analyse_init_qp(x264_t* h, x264_mb_analysis_t* a, int qp)
{
    ...
    if (h->param.QPA1 == 1) {  //全局变量qp_qpa_fixed
        //extern float qp_qpa_org;
        
        a->i_lambda = x264_lambda_tab[(int)qp_qpa_fixed];
        a->i_lambda *= (pow(2.0, (double)(qp_qpa_fixed - qp) / 3.0));
        a->i_lambda2 = x264_lambda2_tab[(int)qp_qpa_fixed];
    }

    else if(h->param.SSIM1 == 1){
        
		a->i_lambda = x264_lambda_tab[(int)qp_qpa_fixed];
		a->i_lambda *= h->fenc->f_lambda_factor[h->mb.i_mb_xy];
		a->i_lambda2 = x264_lambda2_tab[(int)qp_qpa_fixed];
		a->i_lambda2 *= pow(h->fenc->f_lambda_factor[h->mb.i_mb_xy], 2);
}
    else {
