...
#define CLIP_DURATION(f) x264_clip3f(f,MIN_FRAME_DURATION,MAX_FRAME_DURATION)
  ...
  //函数
#define apprI4Log2 x264_template(apprI4Log2)
int apprI4Log2(const double d);
#define filterAndCalculateAverageEnergies x264_template(filterAndCalculateAverageEnergies)
void filterAndCalculateAverageEnergies(pel* pSrc, int srcStride, double* hpEner, int height, int width, int bitDepth);
#define getAveragePictureEnergy x264_template(getAveragePictureEnergy)
double getAveragePictureEnergy(int width, int height, int bitDepth);
#define applyQPAdaptation x264_template(applyQPAdaptation)
void applyQPAdaptation(x264_frame_t* frame);

 //调用全局变量
    //extern float qp_qpa_org;
    
    //qp_qpa_org = qp;



#define ssimqt_init x264_template(ssimqt_init)
extern void ssimqt_init(int width, int height);
#define ssimqt_factor x264_template(ssimqt_factor)
extern void ssimqt_factor(uint8_t* p_src, int width, int height, int stride);
#define ssimqt_qpoffset x264_template(ssimqt_qpoffset)
extern int ssimqt_qpoffset(int posx, int posy, int width, int height, int qp, float* factor);
#define ssimqt_variance x264_template(ssimqt_variance)
