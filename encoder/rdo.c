...
static int rd_cost_mb(x264_t* h, int i_lambda2)//计算16*16宏块的rd-cost
{


	if (h->param.JND1 == 1) {//1
		int16_t* jnd_y_arr = h->fenc->Jnd_arr; //分子
		double jnd_rdo = h->fenc->jnd_rdo - 1.5; //分母
		...
		int i_mb_x = h->mb.i_mb_x;//当前宏块坐标x
		int i_mb_y = h->mb.i_mb_y;//当前宏块坐标x
		i_lambda2 = (jnd_y_arr[i_mb_y * col + i_mb_x] / jnd_rdo) * i_lambda2;
	}
