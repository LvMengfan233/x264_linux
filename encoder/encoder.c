...
static void fdec_filter_row(x264_t* h, int mb_y, int pass)
{
	...
	if (b_measure_quality)
	{
		...
		if (h->param.analyse.b_ssim)
		{
			...
			if (h->param.GAUSSIAN1 == 1) {
				h->stat.frame.f_ssim += x264_pixel_ssim_wxh(&h->pixf,
					h->fdec->plane[0] + 2 + minpix_y * h->fdec->i_stride[0], h->fdec->i_stride[0],
					h->fenc->plane_org[0] + 2 + minpix_y * h->param.i_width, h->param.i_width,
					h->param.i_width - 2, maxpix_y - minpix_y, h->scratch_buffer, &ssim_cnt);
			}
			else
			{
...
static void gaussian_filter_data_clip(uint8_t* psrc, uint8_t* pdst,int width, int height, int stride, double ratio)
{
	...
	for (int i = 0; i < height; i++)
	{
		...
		{
			temp = psrc[i * stride + j] + ratio * psrc[i * stride + j] - ratio * pdst[i * stride + j];
			if (temp > 255)
			{
				pdst[i * stride + j] = 255;
			}
...
static void gaussian_filter_UV(uint8_t* p_src, int width, int height, int stride, uint8_t* p_dst)
{
	...
	double          d_sum_U = 0.0;
	double          d_sum_V = 0.0;
	double* d_gauss = (double*)malloc(sizeof(double) * (2 * N) * N);
	...
	for (int j = radius; j < i_h - radius; j++)
	{
		for (int i = 2 * radius; i < i_w - 2 * radius; i+=2)
		{
			...
			p_dst[j * i_stride + i] = (uint8_t)sum_U;
		}
	}
	for (int j = radius; j < i_h - radius; j++)
	{
		for (int i = 2 * radius + 1; i < i_w - 2 * radius + 1; i+=2)
		{
			...
			p_dst[j * i_stride + i] = (uint8_t)sum_V;
		}
	}
...
static void CLHE(uint8_t *p_src, uint8_t *p_he, int width, int height, int stride, double ratio_he)
{
	...
	// 限制对比度计算
	int LIMIT = (int)(1.0f * total / 255 / 4 * ratio_he);


	int steal = 0;  //因限制而少加的像素个数
	for (int k = 0; k < 256; k++)
	{
		if (tmp[k] > LIMIT) {
			steal += tmp[k] - LIMIT;
	...
	for (int i = 0; i < 256; i++)
	{
		histEQU[i] = (int)(255.0 * C[i] + 0.5);
	}
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			int temp = p_src[i * stride + j];
			p_he[i * stride + j] = (uint8_t)histEQU[temp];
		}
	}
}
#endif
...
//=============================================================gamma校正 Y分量
	if (h->param.GAMMA1 == 1)
	{
		...
		// printf("%d %p %d %d %d %d\n",h->param.pic_cnt,g_src,g_src[0],g_src[h->param.i_width -1],g_src[(h->param.i_height -1) * h->fenc->i_stride[0]],
		// g_src[(h->param.i_height -1) * h->fenc->i_stride[0] + h->param.i_width -1]);
		// h->param.pic_cnt++;
		...
		if (need_gamma) 
		{
			...
			for (int i = 0; i < h->param.i_height; i++)
			{
				for (int j = 0; j < h->param.i_width; j++)
				{
					...
					if (temp > 255)
					{
						p_gamma[i * h->fenc->i_stride[0] + j] = 255;
					}
					else if (temp < 0)
					{
						p_gamma[i * h->fenc->i_stride[0] + j] = 0;
					}
					else
					{
						p_gamma[i * h->fenc->i_stride[0] + j] = temp;
					}
...
//=============================================================直方图均衡化 Y分量
	if (h->param.HistogramEqualization1 == 1)
	{
		...
		uint8_t* p_he = (uint8_t*)malloc(sizeof(uint8_t) * (h->fenc->i_stride[0] * h->param.i_height));
		memcpy(p_he, h->fenc->plane[0], sizeof(uint8_t) * (h->fenc->i_stride[0] * h->param.i_height));
...
//=============================================================高斯滤波 Y分量
	...
	{
		// 对亮度分量进行高斯滤波
		uint8_t* psrc = h->fenc->plane[0];
		uint8_t* pdst = (uint8_t*)malloc(sizeof(uint8_t) * (h->fenc->i_stride[0] * h->param.i_height));
		uint8_t* ptmp = (uint8_t*)malloc(sizeof(uint8_t) * (h->fenc->i_stride[0] * h->param.i_height));
		memcpy(pdst, h->fenc->plane[0], sizeof(uint8_t) * (h->fenc->i_stride[0] * h->param.i_height));
		...
		for (int i = 0; i < h->param.i_height; i++)
		{
			...
			for (int j = 0; j < h->param.i_width; j++)
			{
				// printf("i=%d,j=%d,psrc[i * h->fenc->i_stride[0] + j]=%d,pdst[i * h->fenc->i_stride[0] + j]=%d\n",i,j,psrc[i * h->fenc->i_stride[0] + j],pdst[i * h->fenc->i_stride[0] + j]);
				temp = psrc[i * h->fenc->i_stride[0] + j] + ratio * psrc[i * h->fenc->i_stride[0] + j] - ratio * pdst[i * h->fenc->i_stride[0] + j];
				// printf("%d,",psrc[i * h->fenc->i_stride[0] + j] - pdst[i * h->fenc->i_stride[0] + j]);
				if (temp > 255)
				{
...
//=============================================================高斯滤波 UV分量		
	...
	if (h->param.JND1 == 1) {
		int Lum_width = h->param.i_width;
		int Lum_height = h->param.i_height;
		int Lum_stride = h->fenc->i_stride[0];

		int16_t* Lum_frameCur = h->fenc->Lum_Y;
		int16_t* Lum_bgCur = h->fenc->Lum_bg;
		int16_t* Lum_f2Cur = h->fenc->Lum_f2;
		int16_t* Lum_sjndCur = h->fenc->Lum_sjnd;


		pixel* pSrc_Y = h->fenc->plane[0];
		for (int j = 0; j < Lum_height; j++)
		{
			for (int i = 0; i < Lum_width; i++)
			{
				Lum_frameCur[i] = pSrc_Y[i];
				//Lum_mgCur[i] = 0;
				Lum_bgCur[i] = 0;
			}
			Lum_frameCur += Lum_width;
			//Lum_mgCur += Lum_width;
			Lum_bgCur += Lum_width;
			pSrc_Y += Lum_stride;
		}
		...
		//////Y
		Lum_frameCur = h->fenc->Lum_Y;
		Lum_bgCur = h->fenc->Lum_bg;
		for (int j = 0; j < Lum_height - 4; j++)
		{
			for (int i = 0; i < Lum_width - 4; i++)
			{
				int sum = 0;
				int index = 0;
				for (int m = 0; m < 5; m++)
				{
					for (int n = 0; n < 5; n++)
					{
						sum += Lum_frameCur[i + m * Lum_width + n] * B[index];
						index++;
					}
				}
				Lum_bgCur[i + (j + 2) * Lum_width + 2] = (double)sum / 32;
			}
			Lum_frameCur += Lum_width;
		}
		...
		for (int j = 0; j < Lum_height; j++)
		{
			for (int i = 0; i < Lum_width; i++)
			{
				if (Lum_bgCur[i] <= 128)
				{
					Lum_f2Cur[i] = 14 * (1 - sqrt(Lum_bgCur[i] / (double)127)) + 2;
				}
				else
					Lum_f2Cur[i] = 3 / (double)128 * ((double)Lum_bgCur[i] - 127) + 2;
			}
			Lum_f2Cur += Lum_width;
			Lum_bgCur += Lum_width;
		}
		...
		for (int j = 0; j < Lum_height; j++)
		{
			...
			Lum_sjndCur += Lum_width;
		}

		//Lum_f1Cur = Lum_f1;
		Lum_f2Cur = h->fenc->Lum_f2;
		Lum_sjndCur = h->fenc->Lum_sjnd;

		for (int j = 0; j < Lum_height - 4; j++)
		{
			for (int i = 0; i < Lum_width - 4; i++)
			{
				//if (Lum_f1Cur[i + (j + 2) * Lum_width + 2] > Lum_f2Cur[i + (j + 2) * Lum_width + 2])
				//{
				//    Lum_sjndCur[i + (j + 2) * Lum_width + 2] = Lum_f1Cur[i + (j + 2) * Lum_width + 2];
				//}
				//else
				Lum_sjndCur[i + (j + 2) * Lum_width + 2] = Lum_f2Cur[i + (j + 2) * Lum_width + 2];     //jnd值
			}
		}


		////映射成16*16的宏块级，每个块取16*16个jnd的平均值
		int row = h->param.i_height % 16 == 0 ? h->param.i_height / 16 : h->param.i_height / 16 + 1;
		int col = h->param.i_width % 16 == 0 ? h->param.i_width / 16 : h->param.i_width / 16 + 1;
		int16_t* Jnd_arrCur = h->fenc->Jnd_arr;
		...
		for (int j = 0; j < row; j++)
		{
			for (int i = 0; i < col; i++)
			{
				int sum = 0;
				int ave = 0;
				if (j == row - 1 && h->param.i_height % 16 != 0)
				{
					h_mb_size = h->param.i_height - j * 16;
				}
				if (i == col - 1 && h->param.i_width % 16 != 0)
				{
					w_mb_size = h->param.i_width - i * 16;

				}
				for (int m = j * 16; m < j * 16 + h_mb_size; m++)
				{
					for (int n = i * 16; n < i * 16 + w_mb_size; n++)
					{
						sum += Lum_sjndCur[m * Lum_width + n];
					}
				}
				ave = (sum / (double)256) + 1;
				if (j == row - 1 && h->param.i_height % 16 != 0)
				{
					ave = (ave - 1) * 2 + 1;
				}
				Jnd_arrCur[j * col + i] = ave;
			}
		}
		...
		for (int j = 0; j < row; j++)
		{
			for (int i = 0; i < col; i++)
			{
				a += pow((double)Jnd_arrCur[j * col + i], (double)-1 / 1.9);
			}
		}
		b = a / (row * col);
		b = pow(b, -1.9);
	...
	x264_picture_t* pic_out)
{
	...
	*pp_nal = NULL;
	...
	if (!h->frames.current[0])
		x264_lookahead_get_frames(h);//在这里还没有计算qp_offset
	...
	//    int ctu_in_width = (h->param.i_width / QPA_SIZE) + ((h->param.i_width % QPA_SIZE) == 0 ? 0 : 1);//lcu横向
	//    int ctu_in_height = h->param.i_height / QPA_SIZE + ((h->param.i_height % QPA_SIZE) == 0 ? 0 : 1);//lcu纵向
	//    int num_lcu = ctu_in_width * ctu_in_height;
	//    h->fenc->m_iOffsetCtu = com_malloc_fast(num_lcu * sizeof(pel));
	//    memset(h->fenc->m_iOffsetCtu, 0, num_lcu * sizeof(pel));
	//
	//    int num = h->param.i_width * h->param.i_height;
	//    h->fenc->temp = com_malloc_fast(num * sizeof(pel));
	//    memset(h->fenc->temp, 0, num * sizeof(pel));
	//  
	//    applyQPAdaptation(h);
	//#endif
	...
	/* Write frame */
	if (h->param.JND1 == 1) {    //malloc
		int width = h->param.i_width;
		int height = h->param.i_height;
		h->fenc->Lum_Y = (int16_t*)malloc(sizeof(int16_t) * (width * height));
		h->fenc->Lum_bg = (int16_t*)malloc(sizeof(int16_t) * (width * height));//背景亮度
		h->fenc->Lum_f2 = (int16_t*)malloc(sizeof(int16_t) * (width * height));
		h->fenc->Lum_sjnd = (int16_t*)malloc(sizeof(int16_t) * (width * height));
		int row = h->param.i_height % 16 == 0 ? h->param.i_height / 16 : h->param.i_height / 16 + 1;
		int col = h->param.i_width % 16 == 0 ? h->param.i_width / 16 : h->param.i_width / 16 + 1;
		h->fenc->Jnd_arr = (int16_t*)malloc(sizeof(int16_t) * (row * col));
	}
