 ...
 * For more information, contact us at licensing@x264.com.111
 *****************************************************************************/
...
REALIGN_STACK int main( int argc, char **argv )
{
    ...
    totaltime = (double)(time_end - time_begin) / CLOCKS_PER_SEC;
...
static struct option long_options[] =
{
    ...
    //WJF
    { "hp-aq",              required_argument,       NULL, 0 },
    { "gauss-aq",           required_argument,       NULL, 0 },
    { "gauss-filter",       required_argument,       NULL, 0 },
    { "jnd-al",             required_argument,       NULL, 0 },
...
static int parse( int argc, char **argv, x264_param_t *param, cli_opt_t *opt )
{
    ...
    for( optind = 0;; )
    {
        int c = getopt_long( argc, argv, short_options, long_options, NULL );//getopt_long  解析输入的参数
        if( c == -1 )
            break;
        if( c == OPT_PRESET )
            preset = optarg;
    ...
    x264_param_default( &defaults );//为存储参数的结构体x264_param_t赋默认值
