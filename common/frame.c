...
static x264_frame_t *frame_new( x264_t *h, int b_fdec )
{
    ...
    else /* fenc frame */
    {
        ...
        if (h->param.rc.i_aq_mode)
        {
            ...
            if (h->param.SSIM1 == 1) {    //给指针分配内存空间
                PREALLOC(frame->f_ssim_qp_offset, h->mb.i_mb_count * sizeof(float));
                PREALLOC(frame->f_lambda_factor, h->mb.i_mb_count * sizeof(float));
            }

            if (h->param.QPA1==1){    //给指针分配内存空间
            PREALLOC(frame->f_qpa_qp_offset, h->mb.i_mb_count * sizeof(float));
